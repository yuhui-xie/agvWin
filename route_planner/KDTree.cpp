...
const point_t KDNode::get_x(){ return x; }
...
KDNodePtr KDTree::nearest_(   //
	...
	point_t branch_pt(branch->get_x());
...
KDNodePtr KDTree::nearest_(const point_t &pt) {
	...
	double branch_dist = dist2(root->get_x(), pt);
...
pointIndex KDTree::nearest_pointIndex(const point_t &pt) {
    ...
    return pointIndex(Nearest->get_x(), size_t(*Nearest));
}
...
pointIndexArr KDTree::neighborhood_(  //
    ...
    d = dist2(branch->get_x(), pt);
    dx = branch->get_x().at(level) - pt.at(level);
    dx2 = dx * dx;
