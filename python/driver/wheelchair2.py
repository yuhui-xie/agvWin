...
class Driver:
    ...
    cmd_v = 0; cmd_w = 0; cmd_manual = True
    ...
    def _joystick(self):
        while self.is_running:
            self.joy.update()
            with self.cmd_lock:
                if self.joy.button(7):self.cmd_manual = True
                elif self.joy.button(6):
                    self.cmd_manual = False
                    self.cmd_v = 0
                    self.cmd_w = 0
                if self.cmd_manual:
                    #self.cmd_v = 0.16
                   # self.cmd_v = -self.joy.axis(1)/2.0
                    self.cmd_v = 0
                    self.cmd_w = 0.018
                             
#driver = Driver("/dev/ttyUSB0", 19200)
driver = Driver('com10',19200,0)
def update(v,w):
    driver.update(v,w)
