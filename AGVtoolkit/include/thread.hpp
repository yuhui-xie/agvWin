...
private:
	...
	* @param none
	*/
	void operator=(const CrThread &){}
	/**
	* @brief Denied copy constructor
	* @param none
	*/
	inline CrThread(const CrThread &){}
	...
	*/
	...
	static void* Starter(void* in_thread){
	#endif
		CrThread * thread = static_cast< CrThread * >(in_thread);
		thread->_isRunning = true;
		//thread->run();
		thread->_func();
		thread->_isRunning = false;
...
public:
	...
	*/
	...
	inline bool start(int (BASECLASS::*_func)()){
		if(_isRunning) return false;
		this->_func = _func;
	#ifdef WINDOWS
		_handle = CreateThread( 0x00, 0x00,CrThread::Starter, static_cast< void* >(this), 0x00, 0x00);
		return _handle != NULL;
	#else
		return pthread_create(&_thread, NULL, BThread::Starter, static_cast< void* >(this)) == 0;
	#endif
