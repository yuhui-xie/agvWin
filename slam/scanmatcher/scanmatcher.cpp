...
ScanMatcher::ScanMatcher(): m_laserPose(0,0,0),m_laserAngles(NULL){
	//m_laserAngles=0;
	m_laserBeams=0;
...
void ScanMatcher::setLaserParameters
	...
	*/
    //assert(beams<LASER_MAXBEAMS); //by liao
    if( m_laserAngles) delete[] m_laserAngles;//by liao
    m_laserAngles = new double[beams];//by liao
	m_laserPose=lpose;
	m_laserBeams=beams;
