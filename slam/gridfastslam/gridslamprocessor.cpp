...
namespace GMapping {
	...
	GridSlamProcessor::GridSlamProcessor(const GridSlamProcessor& gsp) 
		:m_particles(gsp.m_particles), m_infoStream(cout){
			...
			cerr << __FUNCTION__ <<  ": trajectories copy.... ";
#endif
...
# ifdef MAP_CONSISTENCY_CHECK
		cerr << __FUNCTION__ << ": performing preclone_fit_test" << endl;
		...
		cerr << __FUNCTION__ <<  ": Number of allocated chunks" << pmap.size() << endl;
		for(PointerMap::const_iterator it=pmap.begin(); it!=pmap.end(); it++)
			assert(it->first->shares==(unsigned int)it->second);

		cerr << __FUNCTION__ <<  ": SUCCESS, the error is somewhere else" << endl;
# endif
		GridSlamProcessor* cloned=new GridSlamProcessor(*this);

# ifdef MAP_CONSISTENCY_CHECK
		cerr << __FUNCTION__ <<  ": trajectories end" << endl;
		cerr << __FUNCTION__ << ": performing afterclone_fit_test" << endl;
...
# endif
	...
	GridSlamProcessor::~GridSlamProcessor(){
		...
		cerr << __FUNCTION__ << ": Deeting tree" << endl;
...
# ifdef MAP_CONSISTENCY_CHECK
		cerr << __FUNCTION__ << ": performing predestruction_fit_test" << endl;
		typedef std::map<autoptr< Array2D<PointAccumulator> >::reference* const, int> PointerMap;
		PointerMap pmap;
		for (ParticleVector::const_iterator it=m_particles.begin(); it!=m_particles.end(); it++){
			const ScanMatcherMap& m1(it->map);
		...
		if (! m_count 
			...
			|| m_angularDistance>m_angularThresholdDistance){
				...
				for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
					...
					it->node->reading = new RangeReading(reading);//code by liao
				}
